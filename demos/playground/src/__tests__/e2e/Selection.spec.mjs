...
test.describe.parallel('Selection', () => {
  ...
  test.fixme(
    'can delete line which starts with element forwards with opt+CMD+delete',
  ...
  test.fixme('can delete line by collapse', async ({page, isPlainText}) => {
    test.skip(isPlainText || !IS_MAC);
    await focusEditor(page);
  ...
  test.fixme(
    `Can't delete forward a Collapsible`,
  ...
  test.fixme(
    `Can't delete backward a Collapsible`,
    async ({page, browserName, isPlainText}) => {
      test.skip(isPlainText);
      if (!IS_MAC) {
        // Do Windows/Linux have equivalent shortcuts?
  ...
  test.fixme(
    'Triple-clicking last cell in table should not select entire document',
    async ({page, isPlainText, isCollab, browserName, legacyEvents}) => {
      test.skip(isPlainText);
  ...
  test.fixme(
    'Can persist the text style (color) from the paragraph',
    async ({page, isPlainText}) => {
      test.skip(isPlainText);
      await focusEditor(page);
  ...
  test.fixme(
    'shift+arrowup into a table, when the table is the only node, selects the whole table',
    async ({page, isPlainText, isCollab, legacyEvents, browserName}) => {
      test.skip(isPlainText);
      test.fixme(browserName === 'chromium' && legacyEvents);
      await focusEditor(page);
