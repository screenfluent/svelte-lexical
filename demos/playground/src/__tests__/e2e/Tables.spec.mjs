...
test.describe.parallel('Tables', () => {
  ...
  test.describe.parallel(`Can navigate table with keyboard`, () => {
    ...
    test(`Horizontal rule inside cell`, async ({
      page,
      isPlainText,
      isCollab,
    }) => {
    ...
    test('Resize merged cells width (1)', async ({
      ...
      const resizerBoundingBox = await selectorBoundingBox(
        page,
        '.SL_Theme__tableCellResizer:first-child',
      );
    ...
    test('Resize merged cells height', async ({
      ...
      const resizerBoundingBox = await selectorBoundingBox(
        page,
        '.SL_Theme__tableCellResizer:nth-child(2)',
      );
      const x = resizerBoundingBox.x + resizerBoundingBox.width / 2;
      const y = resizerBoundingBox.y + resizerBoundingBox.height / 2;
      await page.mouse.move(x, y);
    ...
    test.fixme(
      'Can insert multiple rows above the selection',
      async ({page, isCollab, isPlainText}) => {
        await initialize({isCollab, page});
        test.skip(isPlainText);
        test.skip(isCollab);
    ...
    test.fixme(
      'Can insert multiple rows below the selection',
      async ({page, isCollab, isPlainText}) => {
        await initialize({isCollab, page});
        test.skip(isPlainText);
        test.skip(isCollab);
    ...
    test.describe('with context menu', () => {
      ...
      test.fixme(
        `Can select cells using Table selection and cut them with the context menu`,
        async ({
          page,
          isPlainText,
          isCollab,
    ...
    test(`Cannot insert nested tables`, async ({
      page,
      isPlainText,
      isCollab,
    }) => {
  ...
  test(`Table action menu is hidden when cell overflows`, async ({
    ...
    test.fixme(browserName === 'firefox');
    test.skip(isPlainText || isCollab);
    await initialize({isCollab, page, tableHorizontalScroll: true});
    await focusEditor(page);
